...
import com.training.digitalgoldloan.service.PhysicalGoldTransactionService;

import jakarta.validation.Valid;

@RestController
@RequestMapping("/api/v3/physical_gold_transactions")
public class PhysicalGoldTransactionRestController {
	...
	public ResponseEntity<String> addTransaction(@Valid @RequestBody PhysicalGoldTransaction transaction)
		...
		return ResponseEntity.ok(env.getProperty("PhysicalGoldService.SUCCESS"));
	}
	...
	public ResponseEntity<String> updateTransaction(@PathVariable("transaction_id") Integer transactionId,
		...
		return ResponseEntity.ok(env.getProperty("PhysicalGoldService.UPDATED") + updatedId);
	}
}
